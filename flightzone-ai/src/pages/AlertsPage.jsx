...
const mockSystemAlerts = [
  { id: 's1', type: 'error', message: 'Optimization engine timeout', ts: '2025-04-14 09:12' },
  { id: 's2', type: 'warning', message: 'Aviationstack API response slow (>3s)', ts: '2025-04-14 08:45' },
  { id: 's3', type: 'info', message: 'Backend connectivity restored', ts: '2025-04-14 07:30' },
];

function AlertsPage({ onUnreadCount }) {
  const [alerts, setAlerts] = useState(mockAlerts);
  const [activeTab, setActiveTab] = useState('flight');
  const [systemAlerts] = useState(mockSystemAlerts);

  // Tell the sidebar the initial unread count as soon as the page loads
  useEffect(() => {
  ...
  const markAsRead = (id) => {
    ...
    setAlerts(updated);
    if (onUnreadCount) {
      onUnreadCount(updated.filter(a => !a.is_read).length);
    }
  };
  ...
  return (
    <div style={{ padding: '24px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
        <h1 style={{ color: '#1B2A4A', margin: 0 }}>Alerts & Issues</h1>
        {unreadCount > 0 && (
      ...
      {activeTab === 'flight' && alerts.map(alert => (
        <div key={alert.id} style={{
          ...
          background: alert.is_read ? '#F3F4F6' : 'white',
          padding: '16px',
          marginBottom: '12px',
          borderRadius: '6px',
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{
              background: getBadgeColor(alert.severity),
              color: 'white',
              padding: '2px 10px',
              borderRadius: '12px',
              fontSize: '12px',
            ...
            <span style={{ fontSize: '12px', color: '#6B7280' }}>{alert.timestamp}</span>
          </div>
          <p style={{ margin: '8px 0', color: '#1F2937' }}>{alert.message}</p>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: '12px', color: '#6B7280' }}>Flight: {alert.flightId}</span>
            {!alert.is_read ? (
              <button onClick={() => markAsRead(alert.id)} style={{
              ...
              <span style={{ fontSize: '12px', color: '#6B7280' }}>✓ Read</span>
            )}
          </div>
        </div>
      ))}
      ...
      {activeTab === 'system' && systemAlerts.map(sa => {
        ...
        return (
          <div key={sa.id} style={{
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ background: color, color: 'white', padding: '2px 10px', borderRadius: '12px', fontSize: '12px', fontWeight: 'bold' }}>
                {label}
              </span>
              <span style={{ fontSize: '12px', color: '#6B7280' }}>{sa.ts}</span>
            </div>
            <p style={{ margin: '8px 0', color: '#1F2937' }}>{sa.message}</p>
          </div>
        );
      })}
    </div>
